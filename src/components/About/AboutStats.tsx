import type { AboutDetail, AboutStat } from '@/types/content';

export default function AboutStats({ stats, details }: { stats: AboutStat[]; details: AboutDetail[] }) {
    return (
        <>
            <div className="grid grid-cols-2 border border-(--border)">
                {stats.map((stat, index) => {
                    ...
                    return (
                        <div key={`${stat.label}-${index}`} className="p-4 sm:p-7" style={{ borderRight: isEvenCol ? 'none' : '1px solid var(--border)', borderBottom: isLastRow ? 'none' : '1px solid var(--border)' }}>
                            <div style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(36px, 8vw, 56px)', fontWeight: 300, color: 'var(--white)', lineHeight: 1, marginBottom: '8px' }}>
                                {stat.value}
            ...
            <div style={{ marginTop: '3px', border: '1px solid var(--border)', display: 'grid', gridTemplateColumns: '1fr' }}>
                {details.map((item, i, arr) => (
                    <div key={`${item.label}-${i}`} className="p-4 sm:p-5" style={{ borderBottom: i < arr.length - 1 ? '1px solid var(--border)' : 'none' }}>
